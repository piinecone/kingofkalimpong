...
function Awake(){
   inputSender = GetComponent(InputSender) as InputSender;
}
...
function FixedUpdate () { 
  ...
  if (groundedL || StrictMode == 2){
    ...
    inputSender.AddForceAtPosition(WheelL.transform.up * -m_antiRollForce, WheelL.transform.position);
  } else if (StrictMode == 1){
    //rigidbody.AddForce(WheelL.transform.up * -m_antiRollForce);
    inputSender.AddForce(WheelL.transform.up * -m_antiRollForce);
  } if (groundedR || StrictMode == 2){
    //rigidbody.AddForceAtPosition(WheelR.transform.up * m_antiRollForce, WheelR.transform.position); 
    inputSender.AddForceAtPosition(WheelR.transform.up * m_antiRollForce, WheelR.transform.position); 
  } else if (StrictMode == 1){
    //rigidbody.AddForce(WheelL.transform.up * m_antiRollForce);
    inputSender.AddForce(WheelL.transform.up * m_antiRollForce);
  }
